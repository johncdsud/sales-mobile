...
function buscarFornecedor(req, res) {
    model.buscarFornecedor((err, data) =>{
        if (err) 
            return res.json(err);
        res.render('../app/views/fornecedor.ejs', { fornecedor: data});
    })
}
...
function buscarFornecedorPorId(req, res) {    
    model.buscarFornecedorPorId(req.params.codigo, (err, data) =>{
        if (err) 
            return res.json(err);
        res.render('../app/views/alteraFornecedor.ejs', { forn: data[0]});
    })
}
...
function novoFornecedor(req, res) {
    model.buscarFornecedor((err,data) => {
        if (err)
            return res.json(err);
         res.render('../app/views/novoFornecedor.ejs',{fornecedor: data});
    });
}
