...
function incluirItemEstoque() {
    movEstoque.dataMovimento = $('datamov').value;
    movEstoque.numeroPedido = $('pedido').value;
    ...
    item.nomeProduto = $(`codigo-produto-${item.codigoProduto}`)
...
function incluirLayout(item) {
    ...
    tdPedido.textContent = movEstoque.numeroPedido;
...
function setErrors(movEstoque, item) {
    ...
    if (movEstoque.numeroPedido == "" || movEstoque.numeroPedido == null)
...
// chama a api
function salvar() {
    var url = `${window.location.origin}/estoqueEntrada/novo`;
